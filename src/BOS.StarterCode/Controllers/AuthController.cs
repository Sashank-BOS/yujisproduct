    ...
    public class AuthController : Controller
    {
                ...
                if (result.IsVerified)
                {
                    ...
                    if (roles != null && roles.Count > 0)
                    {
                        foreach (UserRole userRole in roles)
                        {
                            ...
                            rolesString = (!string.IsNullOrEmpty(rolesString)) ? ("," + rolesString + role.Name) : (rolesString + role.Name);
                        }
                    }
        ...
        public async Task<ActionResult> RegisterUser(RegistrationModel registerObj)
        {
            ...
            if (result.IsSuccessStatusCode)
            {
                ...
                helper.SendEmail(registerObj.EmailAddress, parameters);

                return View("Index");
            }
            else
            {
                ModelState.AddModelError("CustomError", "We are unable to register you right now. Please try again in sometime. Thank you.");
                return View("Index", new AuthModel());
            }
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (userResponse.Users != null)
                {
                    ...
                    if (forcePasswordChange.IsSuccessStatusCode)
                    {
                        ...
                        helper.SendEmail(forgotPasswordObj.EmailAddress, parameters);
                    }
